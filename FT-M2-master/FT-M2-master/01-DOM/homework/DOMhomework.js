...
// Tu código acá:
  var nombre = "ANG31"
  var spa = document.querySelector("#createdBy")
  spa.innerHTML = spa.innerHTML + nombre
...
function displayToDos() {
  ...
  var rest  = buildToDos(toDoItems)
  for (let a = 0; a < rest.length; a++) {
    toDoContainer.appendChild(rest[a])
  }
}
...
function addToDo() {
  ...
  var tod = new ToDo(val)
  toDoItems.push(tod)
  var imp = document.getElementById('toDoInput')
  imp.value = ""
  displayToDos()
}
...
// En este paso vamos a utilizarlo para encontrar el index del item que disparó el evento (Esta parte ya se encuentra desarrollada pero está comentada dentro de la función por lo que va a ser necesario que la descomenten)]
